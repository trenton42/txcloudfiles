...
'''
    ...
    Trivial example of how to enable logging on a container. See:
    
    http://docs.rackspace.com/files/api/v1/cf-devguide/content/Acess_Log_Delivery-d1e2200.html
...
def _got_session(session):
    ...
    def _ok((response, v)):
        ...
        print '> enabled logging for: %s' % container_name
        reactor.stop()
    print '> sending request'
    # 'container' here is any name of an existing empty container. Can be a Container() object if you like.
    session.enable_container_logging(container=container_name).addCallback(_ok).addErrback(_error)
